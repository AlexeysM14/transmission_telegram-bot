...
- `15` — добавить URL подписки Hysteria 2 и включить её автоматическое обновление.
...
Вместо отдельного профиля можно выбрать пункт `15` и вставить HTTP(S)-ссылку на подписку. Поддерживается
обычный текстовый список и список в Base64; используется первая корректная ссылка `hysteria2://`, `hy2://`
или `hysteria2+realm://`. URL хранится с правами `0600`, а systemd-таймер проверяет подписку каждые 15 минут.
При изменении профиля конфигурация записывается атомарно и клиент Hysteria 2 автоматически перезапускается.
Переход обратно на профиль через пункт `13` отключает обновление по ранее сохранённой подписке.
При добавлении URL CLI сразу загружает подписку и завершает настройку только в том случае, если ответ содержит
поддерживаемый профиль. Поэтому одного корректного вида URL недостаточно: сервер должен быть доступен с хоста бота
и возвращать текстовый либо Base64-список ссылок Hysteria 2.
...
- `📋 Торренты` — постраничные списки, фильтры, поиск и быстрые действия без лишних сообщений в чате.
...
- `LIST_LIMIT` — размер страницы в списках торрентов (по умолчанию `25`, в интерактивном списке не более 8 элементов на страницу).
